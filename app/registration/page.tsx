...
import { apiRegister, isApiAuthEnabled, getGoogleAuthUrl } from '../../lib/authApi'
...
export default function RegistrationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : t.auth.alreadyExists
      setError(msg)
      toast.error(msg)
      setIsLoading(false)
    }
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-white">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6">
        ...
        <h1 className="text-2xl font-semibold text-[#24252C] mb-8 text-center" suppressHydrationWarning>
          {isClient ? t.auth.registration : 'Registration'}
        </h1>
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-[331px] space-y-4 mb-6">
          ...
          <div className="flex justify-center">
            <label className="cursor-pointer">
              ...
              <div className="w-20 h-20 rounded-full bg-gray-200 overflow-hidden border-2 border-[#5F33E1] flex items-center justify-center">
                {avatarPreview ? (
                  <img src={avatarPreview} alt="" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-2xl text-[#6E6A7C]">+</span>
                )}
              </div>
            </label>
          </div>
        ...
        <Link href="/login" className="text-sm text-[#5F33E1]">
          {t.auth.login}
        </Link>
      </div>
    </div>
